\label{sec:clusters}
In this section we try to compare the performance of RMSD task on different HPC resources to examine the robustness of the methods we used for our performance study and to also make sure that the results are resource independent.
...
These cases include: splitting the trajectories with blocking collective communications in MPI, splitting the trajectories with Global Arrays for communications, and MPI-based parallel HDF5.
...
When we split the trajectories the scaling follows the same pattern on both Comet and SuperMIC with Global Arrays and without Global Arrays.
Both Comet and SuperMIC scale very well using Global Arrays. 
RMSD task still scales on both clusters without Global Arrays; however, scaling is far from ideal scaling due to the communication cost (Refer to section \ref{splitting-traj} and Figures \ref{fig:MPIranks-split} and \ref{fig:MPIwithIO-split-SuperMIC}). 
...
Presents results show that Global Arrays work better than MPI blocking collective communication.
...
\end{subfigure}
%
\caption{Comparison of the performance of the RMSD task with MPI (\text{$\tcomp/\tIO \approx 0.3$}) when the trajectories are split using global 
array and without Global Arrays across different clusters (SDSC Comet, SuperMIC). 
...
Overall, the scaling of the RMSD task is better on SuperMIC.
...
\end{subfigure}
%
\caption{Comparison of the performance of the RMSD task with MPI (\text{$\tcomp/\tIO \approx 0.3$})
...
and results are communicated back to rank 0 (communications included). $N_{\text{P}}/N_{\text{nodes}}$ means that number of processes used for the task is equal on all compute nodes.
...
Five repeats were performed to collect statistics and this is typical data from one run of the 5 repeats. Compute \tcomp, IO \tIO, communication \tcomm, ending the for loop \text{$t_{\text{end\_loop}}$},
...
These plots together with Table \ref{tab:comp-IO-scaling} allow both quantitative and qualitative comparison of the compute and I/O time scaling across different resources and for different test cases.
...
When the trajectories are split and Global Arrays is used for communication both Comet and SuperMIC show similar performance at small number of cores and the their performance difference increases as the number of cores increases. 
...
\cmidrule(r){5-12}
  ...
  \midrule
    SuperMIC & MPI & Single & \begin{tabular}{c} \tIO \\ \tcomp  \end{tabular} & \begin{tabular}{c} $791 \pm 5.22$ \\ $225 \pm 5.4$ \end{tabular} & \begin{tabular}{c} $49 \pm 3.45$ \\ $11 \pm 0.75$ \end{tabular} & \begin{tabular}{c} $29 \pm 1.3$ \\ $6 \pm 0.35$ \end{tabular} & \begin{tabular}{c} $26 \pm 9.19$ \\ $4 \pm 0.48$ \end{tabular} & -- & -- & -- & --\\  
  \midrule
