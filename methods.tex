...
\begin{gather}
  \label{eq:Compute-I/O}
    \frac{t_{comp}^{frame}}{t_{IO}^{frame}} \approx \frac{\tcomp}{\tIO} 
 \end{gather}
...
\begin{tabular}{c c}
  ...
  \midrule
    ...
    $t_{comp}$ &  $sum(t_{comp}^{frame}) \approx N_{frames} \times t_{comp}^{frame}$\\
    $t_{IO}$ & $sum(t_{IO}^{frame}) \approx N_{frames} \times t_{I/O}^{frame}$\\
