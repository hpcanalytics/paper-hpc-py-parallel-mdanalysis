...
The total time to completion for a single process on $N$ cores is $t_{\text{N}}$, which is mathematically equivalent to
$ t_{\text{RMSD}} + \tcomm \equiv t_{\text{N}}$.

\subsection{Performance Measurement}
We also recorded the total time to solution $t_{\text{total}}(N)$ with $N$ MPI processes on $N$ cores (which is effectively
$t_{\text{total}}(N) \approx \max t_{\text{N}}$). 
