...
\begin{tabular}{c c}
  ...
  \midrule
    ...
    $t_{\text{comm/GA}}$ &  $t_{\text{L5}}+t_{\text{L6}}+t_{\text{L7}}+t_{\text{L8}}$  (in Alg.~\ref{alg:GA}) \\
  ...
  \bottomrule
\end{tabular}
\caption[Summary of measured timing quantities.]
{Summary of measured timing quantities.
Timings are collected for the specified line numbers in the code, labeled as $t_{\text{L$n$}}$ where $\text{L$n$}$ refers to the line number in the corresponding algorithm (Algorithm~\protect\ref{alg:RMSD} unless noted). 
