...
\obnote{not clear to me what you want to say here ? do you want to say that the average ratio is approximately the ratio of the averages?} \mknote{No. compute time per frame and I/O time per frame should not change in parallel and in serial. Therefore we can say that this ratio can be estimated using serial version of the code}\obnote{I/O time per frame \emph{should} not change, but we know that they do for larger $N$! I understand that in an ideal scenario they should not change. Therfore, it makes sense to use the serial version. Because this is already expressed in the preceeding sentence, I will remove the equations below in the next commit.}
\begin{gather}
