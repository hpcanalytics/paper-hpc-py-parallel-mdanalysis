\label{methods}
\gpnote{This section is more like a presentation of the libraries used, not how they were used. The second paragraph is called applications, while the section methods. Also, it is not clear what methods you used and how.}
...
In shared-memory systems, regardless of the implementation, the shared-memory paradigm eliminates\gpnote{It does not eliminate synchronization. It does not requires messages since all threads of execution have access to the same memory space. Synchronization is needed for consistency.} the synchronization that is required when message-passing is used to access shared data. 
\gpnote{I do not understand where the disadvantage is. Can you clarify it a bit?}
...
The GA library keeps track of all these memory locations by recording a list\gpnote{Is this list distributed of centralized? Which process is the owner?} of them when a global array is created. 
When a process tries to access a block of data, it first does a decomposition of the request into individual blocks representing the contribution to the total request from the data held locally on each process\gpnote{Data are held by processes, not processors. Also the citation is missing.}\cite{PNNL:2017}. 
...
OpenIB/infiniband uses sys5 shmem\gpnote{You want to avoid language that is OS specific. You say in the introduction that scientist are not system administrators. sys5 shmem is very specific even for me.} within the node and will use the infiniband network card to communicate between nodes.
...
This is advantageous to avoid multiple processes to compete over accessing the same file on disk\gpnote{I am not sure what you want to say here. Are the processes accessing the file atomically, i.e. when on process access it all others are blocked? Doesn't that defeat the purpose of parallel I/O}. 
