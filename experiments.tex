...
A more detailed analysis showed that the RMSD computation, and to a lesser degree the I/O, considered on their own scaled well beyond 50 cores (yellow and blue lines in Figure~\ref{fig:ScalingComputeIO}). Communication (red line in Figure~\ref{fig:ScalingComputeIO}) and the initial file opening (gray line in Figure~\ref{fig:ScalingComputeIO}) started to dominate beyond 50 cores.
...
In the run shown in Figure~\ref{fig:MPIranks}, 62 ranks out of 72 took about $60sec$ (the stragglers) whereas the remaining ranks only took about $20sec$. 
...
There was no clear pattern in which MPI rank would be a straggler, neither could we trace stragglers to specific cores or nodes. 
...
\subsection{Effect of average compute over average IO ration on Performance}
\label{sec:bound}
...
These generally agreed with the theoretical prediction in Table~\ref{tab:load-ratio}. The $\overline{\tcomp}/\overline{\tIO}$ ratio for higher workloads (with factor $X$) should be $X$ times the $\overline{\tcomp}/\overline{\tIO}$ of the intial workload. On average, the I/O workload of each rank is $N_{\text{b}} \times \tIO $, where $N_{\text{b}} = N_{\text{frames}}^{total}/N$ is the number of frames processed by each MPI rank for $N$ processes, is independent of $X$. While the workload for the computation is $X \times N_{\text{b}} \times \tcomp$, and hence the ratio is $X \times \overline{\tcomp}/\overline{\tIO}$.
...
The RMSD calculation consistently scaled up to larger numbers of cores ($N=56$ for $70\times$ RMSD).
...
When result arrays communicated back to the master process (rank 0), some processes took much longer as compared to others. 
...
Motivated by this section's results, we investigated the influence of the ratio of compute to communication ($\overline{\tcomp}/\overline{\tcomm}$) on performance in~\ref{sec:tcomm}.
We found evidence for the hypothesis that a larger ratio was beneficial, provided I/O costs could also be reduced, as discussed in the next section.
...
In order to improve performance we employed strategies to avoid file access competition across ranks when the $\overline{\tcomp}/\overline{\tIO}$ ratio was small.
...
In all the previous benchmarks all processes were using a shared trajectory file. ``Subfiling'' is a mechanism previously used for splitting a large multi-dimensional global array to a number of smaller subarrays. Each smaller array is saved in a separate file. As a result, it reduces the file system control overhead by decreasing the number of processes concurrently accessing a shared file~\cite{scalable-IO, scalable-IO1}.
...
Tthe trajectory file was split into $N$ segments, one for each process, with each segment having $N_{b}$ frames. 
...
This work indicated that MPI traffic (inter-process communication) can be affected by increasing I/O, and in particular, a few MPI processes were always delayed by 1-2 orders of magnitude more than the median time. Thus suggesting that our observed stragglers with large variance in the MPI communication might be due to interference with the I/O requests on the same network.
