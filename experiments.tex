...
single node on other machines\gpnote{Please specify which other machines}), using either dask or MPI \cite{Khoshlessan:2017ab}. 
...
\gpnote{Are the data in figure 2 from Comet and Stampede? Why Stampede and not SuperMic, Bridges, and Comet? Those are the system you say the experimentation was done.}
...
\emph{TACC Stampede} and \emph{TACC Data Analytics System Wrangler} (data not shown)\gpnote{I am not sure you want to say that you do not show data. You can include them in an appendix, or provide a link to a repo with all the data and figures.}. There was also no clear pattern in which certain MPI
...
reveal an obvious pattern). Therefore, the phenomenon of stragglers in the RMSD test appears to be independent from the underlying hardware\gpnote{I would say resource independent, not hardware. Hardware opens a can of questions which is very difficult to answer with the data we have.}.
...
result arrays\gpnote{Can you quantify large? Anything in terms of elements, Bytes or any other metric.}, which is costly for multiple ranks (Figure \ref{fig:comparison-t_comm-dihedral})\gpnote{Please check the comment in caption}, the speed-up curve (Eq.~\ref{eq:speedup}) in Figure \ref{fig:MPIspeedup-dihed}
...
\caption{Comparison of communication cost for different number of
  ...
  workload (with \emph{communications included}).\gpnote{I am really confused with this figure. How many GBs of data did you move and there was a delay of 1.5 hours? Was it more 10TBs? (Assuming you were able to use 20Gbps from IB $20Gbps*5000sec=10^{14}=12500000000000B=12500000000000/2^{40}TB\simeq11.6TB$)}}
