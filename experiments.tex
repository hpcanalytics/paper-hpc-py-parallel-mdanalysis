...
In fact, the $\overline{\tcomp}/\overline{\tIO}$ ratio for the higher workloads (with factor $X$) should be $X$ times the $\overline{\tcomp}/\overline{\tIO}$ for the $1\times$ workload (in agreement with the theoretical prediction in Table \ref{tab:load-ratio}) because the average the I/O workload of each rank is $N_{\text{b}} \times \tIO $, which is independent of $X$. 
