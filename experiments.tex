...
As in the previous work we also observed very poor strong scaling performance (Figures \ref{fig:MPIscaling}, \ref{fig:MPIspeedup} and \ref{fig:MPIwithIO-Bridges}) beyond a single node.
...
and we were not able to achieve good scaling above a single compute node. 
We hypothesized that decreasing the relative I/O load with respect to compute load would also reduce number of stragglers. 
We believe as the task becomes more compute bound the processes will not constantly be performing I/O and instead, I/O is interleaved with longer periods of computation.
...
On average, each rank\textsc{\char13}s workload is $N_{\text{frames}} \times t_{\text{I/O}}^{\text{frame}} $ (where $N_{\text{frames}}=N_{\text{frames}}^{\text{total}}/N$ is the
number of frames per trajectory block, i.e., the number of frames
processed by each MPI rank for $N$ processes) for I/O, and
$X \times N_{\text{frames}} \times t_{\text{comp}}^{\text{frame}}$ for the RMSD calculation. 
...
These delayed tasks impact performance as well and as a result speed-up is far from ideal (Figure \ref{fig:MPIspeedup-split}).
...
As a result, communication time would not be problematic even if compute to communication ratio is small (Figure \ref{fig:tcomp_tcomm_ratio}, 1-24 cores represents a single compute node on SDSC Comet).
However, beyond a single node, communication overhead becomes a bottleneck and effect of \tcomp/\tcomm becomes very important (Figure \ref{fig:tcomp_tcomm_ratio}, 24-72 cores represents multiple compute node on SDSC Comet).
