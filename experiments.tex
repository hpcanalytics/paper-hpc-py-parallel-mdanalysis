...
result arrays (e.g. $\approx 2GB$ with 8 processes and $\approx 0.24GB$ with 72 processes)\gpnote{Can you quantify large? Anything in terms of elements, Bytes or any other metric.}\mknote{done!} , which is costly for multiple ranks (Figure \ref{fig:comparison-t_comm-dihedral}), the speed-up curve (Eq.~\ref{eq:speedup}) in Figure \ref{fig:MPIspeedup-dihed}
...
This is mainly because both communication and I/O are very small with respect to compute time ($\tcomp/\tIO \approx 100$ and \mknote{Need to quantify t\_comp/t\_comm}). 
Although, there are stragglers due to communication (\mknote{Add figure showing stragglers}) their effect on performance is not crucial mainly because compute to communication ratio is very large. 
