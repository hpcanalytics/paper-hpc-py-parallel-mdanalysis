...
though they still contribute to large variations in timing across different ranks and thus to somewhat erratic scaling.\gpnote{What is erratic scaling? Can you quantify it?}\mknote{It just means that the scaling is not linear and goes up \& down}
...
Based on the results shown in section \label{Splitting}, although we overcame the I/O effect by splitting the trajectory the task is far from ideal scaling without global array.
...
we were not able to achieve near ideal scaling even when I/O is not a bottleneck anymore.
However, Dihedral featurization turned out to be compute bound and we were able to achieve near ideal scaling even when we did not use global array in our benchmark.
This is mainly because both communication and I/O are very small with respect to compute time. 
Although, there are stragglers due to communication their effect on performance is not crucial due to the fact that compute to communication ratio is very large. 
We therefore, hypothesized that decreasing the relative communication load with respect to compute load would also reduce the stragglers. 
