...
\obnote{What data do we show to prove the statement that we have stragglers on other machines? In principle we would want the same kind of figure as Fig~\ref{fig:MPIwithIO}---if we already have that, I would include that in Appendix. Alternatively, add similar figures like Fig \ref{fig:comparison_efficiency} for \emph{Bridges} and \emph{SuperMIC}.}
...
The present results show that contention for file access impact the performance. 
