...
We therefore performed more experiments with more detailed timing information (see section \ref{sec:methods}) on \emph{SDSC Comet} (described in this section) and two other supercomputers (summarized in section \ref{sec:clusters}) in order to better understand the origin of the stragglers. 
...
The ranks that took much longer to complete than the typical execution time of the other ranks were the stragglers that hurt performance and prolong job completion.
...
Although the data in Figure~\ref{fig:MPIranks} represented one repeat and in other repeats different number of ranks were stragglers, the overall hypothesis was confirmed by the averages over all ranks in five independent repeats (Figure~\ref{fig:ScalingComputeIO}).
...
\subsubsection{I/O Leads to Stragglers}
 ...
 \begin{figure}[!htb]
   ...
   \end{subfigure}
   \caption{Comparison on the performance of RMSD task with MPI with I/O and without I/O ($\RcompIO \approx 0.3$) on \emph{SDSC Comet}.
...
\begin{figure}[!htb]
  ...
  \caption{Performance of the RMSD task with MPI using Global Arrays ($\RcompIO \approx 0.3$) on \emph{SDSC Comet}.
    ...
    In (d), compute \tcomp, IO \tIO, communication \tcomm, access to the whole Global Arrays by rank 0, $t_{\text{Access\_Global\_Array}}$, ending the for loop $t_{\text{end\_loop}}$,
...
This way, each process would access its own trajectory segment file without competing for file access. 
...
\caption{Comparison on the performance of the RMSD task ($\RcompIO \approx 0.3$) on \emph{SDSC Comet} when the trajectories are split.
  ...
  (c) Speed-up.
  (a-c) The error bars show standard deviation with respect to mean.
  (d-e) Compute \tcomp, IO \tIO, communication \tcomm, access to the whole Global Arrays by rank 0, $t_{\text{Access\_Global\_Array}}$, ending the for loop $t_{\text{end\_loop}}$, opening the trajectory $t_{\text{opening\_trajectory}}$, and overheads $t_{\text{overhead1}}$, $t_{\text{overhead2}}$ per MPI rank; see Table \ref{tab:notation} for the definitions.
  Note: In serial, there is no communication and no data points are shown for $N=1$.}
...
These results showed that contention for file access clearly impacted performance. 
...
In~\ref{sec:chainreader} we investigated if existing functionality in MDAnalysis that can create virtual trajectories from trajectory segments could benefit from the subfiling approach. \mknote{The problem with chainreader was that each process was accessing all other segments although it did not need to use them for any calculations. This does not mean that if we have many more small segments than the Number of processes we can not achieve ideal scaling. I do believe that we can achieve ideal scaling by having a balance load and avoiding chain-reader. I have done this on Mashteryoda and I got very good scaling.}
We found some improvements in performance but discovered limitations in the design that first have to be overcome before equivalent performance can be reached. \mknote{I believe we should remove this sentence. It is not true.}
...
\begin{figure}[!htb]
  ...
  \caption{Performance of the RMSD task with MPI-based parallel HDF5 ($\RcompIO \approx 0.3$) on \emph{SDSC Comet}.
    ...
    Five repeats were performed to collect statistics.
    (a-c) The error bars show standard deviation with respect to mean.
    (d) Compute \tcomp, IO \tIO, communication \tcomm, ending the for loop $t_{\text{end\_loop}}$,
...
The data indicated that increase in the duration of both MPI communication and trajectory file access lead to large variability in the run time of individual MPI processes and ultimately poor scaling performance beyond a single node.
...
The reason for this behavior is related to POSIX consistency requirement that relates to POSIX I/O API and POSIX I/O semantics, which can have adverse effects on scalability and performance.
