...
One obvious approach when using the Lustre parallel file system is to increase the number of stripes, i.e., the number of copies of a file that are stored on different storage targets (OSTs).
But because in our previous work we did not see performance improvement with varying the stripe count \cite{Khoshlessan:2017ab} we decided to just use the system default.
...
\paragraph{Subfiling}
We tested subfiling (see Section~\ref{sec:splitting-traj}) with GA to reduce the initial delay due to trajectory opening.
