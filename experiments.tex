...
For other ranks, \tcomm varies across different ranks and for a few ranks $\tcomm < 10~\text{s}$ or is barely measurable. 
...
\obnote{The subfigure \protect{\ref{fig:ScalingComputeIO}} is great but the large contribution of \textbf{Opening trajectory} is not at all visible in the example \protect{\ref{fig:MPIranks}}. Why? If the error bars on the opening trajectory are small then all examples for MPI ranks should show times for \textbf{Opening trajectory} with magnitude greater than the \textbf{compute} time. Please explain briefly and find an example that is consistent.}\mknote{It is shown. 1d is one repeat out of five. In some repeats this is small and some is large. And what I am showing in 1c is the avg of all repeats along with the standard deviation. I changed it to a case that show this is large.}
\label{fig:MPIwithIO}
\end{figure} 
...
On average, each rank\textsc{\char13}s workload is $N_{\text{frames}} \times \tIO $ (where $N_{\text{frames}}=N_{\text{frames}}^{total}/N$ is the
