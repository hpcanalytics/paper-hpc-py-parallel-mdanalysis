...
\obnote{This paragraph needs to be rewritten to better motivate this paper. For instance, they might not discuss stragglers because they don't see them so we just have to formulate our motivation a bit more carefully. We need to make clearer that we provide general guidelines that some packages might have already overcome but that have to the best of our knowledge been clearly elucidated in the literature.}
...
In the present work we want to better understand general requirements for efficient parallel analysis of MD trajectories in order to implement efficient parallel algorithms in \package{MDAnalysis} but to also provide more generally guidance for similar developments in other libraries.
