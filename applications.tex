...
When a process tries to access a block of data, request is first decomposed into individual blocks representing the contribution to the total request from the data held locally on each process \cite{PNNL:2018}.\obnote{This ``personal communication'' reference is broken; e.g. no author. Can you replace it with something fully citable --- a paper or docs? Personal communication citations are not very useful but if you want to keep it, write it in text: (A.U. Thor, personal communication). The journal might still not allow it.} 
...
HDF5 is a structured self-describing hierarchical data format which is the standard mechanism for storing large quantities of numerical data in Python \cite{pythonhdf5, HDF5_url}.\obnote{The reference \emph{HDF5\_url} \cite{HDF5_url} is broken. Please fix. If it is just a URL, add it in text.}
