...
The \emph{Global Arrays} (GA) toolkit provides users with a language interface that allows them to distribute data while maintaining the type of global index space and programming syntax similar to what is available when programming on a single processor \cite{GA}. The Global Arrays toolkit allows manipulating physically distributed dense multi-dimensional arrays without explicitly defining communication and synchronization between processes.
Communication is configured by a runtime, which defaults to the \emph{Communication runtime for Extreme Scale} (ComEx) \cite{Daily:2014aa}.
ComEx uses share memory for intra-node communication and we employed ComEx with MPI for inter-node communication.
...
It also contains functions to destroy arrays (\texttt{ga\_destroy()}) and to free up the memory originally allocated to them \cite{GAiN}.
