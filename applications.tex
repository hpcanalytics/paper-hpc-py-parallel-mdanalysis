...
\begin{algorithm}[ht]
	...
	\begin{algorithmic}[1]
		...
		\State xref0 $\leftarrow$ Reference atom group position
...
\begin{algorithm}[ht]
	...
	\hspace*{\algorithmicindent} \textbf{Output:} Calculated RMSD arrays
	\begin{algorithmic}[1]
		\Procedure{$Block\_RMSD$}{dataset, ref, start, stop}                       
		\For{ start $\leq iframe < $ stop }
		\State $results[iframe] \leftarrow RMSD(dataset[iframe], ref)$
		\EndFor
		\State \Return results
		\EndProcedure
		\\        
		...
		\State rank $\leftarrow$ rank ID
		\State xref0 $\leftarrow$ Reference atom group position
                \State f $\leftarrow$ Open HDF5 file with MPIO for reading
                \State dataset $\leftarrow$ Get dataset 'pos' from f
		\State out $\leftarrow$ Block\_RMSD(dataset, xref0, start=start, stop=stop)
		\\
		\State Gather(out, RMSD\_data, rank\_ID=0)
		\State MPI Finalize
	\end{algorithmic}
