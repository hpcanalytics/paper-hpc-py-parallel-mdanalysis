...
\package{MDAnalysis} supports many common file formats of simulation packages including CHARMM, Gromacs, Amber, and NAMD and the Protein Data Bank format and enables accessing data stored in trajectories. 
\package{MDAnalysis} utilizes fast numeric/algebraic libraries such as ATLAS, LAPACK, or MKL to improve speed and powerful Python libraries such as NumPy and SciPy.
...
\gpnote{I do not understand this sentence. What do you mean by other factors?}\mknote{I say other factors like LOC, number of function invocation, ..}\gpnote{Okay, I see that, but how do these factors add to Python's attractiveness? Why not some other high level interpreted language, like Java?}\mknote{Why not? When writing code in python you need less number of lines to code something and therefore, it is easier to program. Also Java might have these features too. But we use python because mdanalysis is in python}\gpnote{Okay! But still the sentence is difficult to parse. I suggest: "In addition to Python\textsc{\char13}s interactive nature, other factors like lines of codes (LOC), number of function invocation, and development time add to its attractiveness and clarifies why parallel programming models, such as MPI or MapReduce, should be supported."}. 
In addition, Python\textsc{\char13}s interactive nature, and other factors like lines of codes (LOC), number of function invocation, and development time, add to its attractiveness and clarifies why it is a good investment to extend Python use to message-passing parallel programming applications.
...
The GA library keeps track of all these memory locations by recording a list\gpnote{Is this list distributed of centralized? Which process is the owner?}\mknote{All the processes have access to it and in that way will get aware of any change in the data of other processes}\gpnote{'Of them'? You answered the question who has access. I am asking who is creating this list? Is it rank 0 for example?}\mknote{Yes a list of them. When you create a global array you will have an array for each process and it is located in the local memory space of each process. So each process has a copy of the whole array for itself.}\gpnote{So every process creates this list. Can you make it a bit more clear in the text?} of them when a global array is created. 
...
HDF5\gpnote{This is a very nice oportunity to introduce HDF5. More details about HDF5 would be nice. If you assume that the reader does not know MPI4py, most probably will not know HDF5} \mknote{Previously I had more detail in this section and Oliver suggested to summarize and avoid too much detail. What type of info do you think will be good to be added here?}\gpnote{I agree with Oliver, but you should at least give an example of details that HDF5 handles. }\mknote{Can you please add this yourself. I do not know know what you have in mind exactly}\gpnote{better feel free to remove these}
itself handles nearly all the details involved with coordinating file access when the file is opened through ``mpio" driver.
\gpnote{I am not sure what you want to say here. Are the processes accessing the file atomically, i.e. when one process access it all others are blocked? Doesn't that defeat the purpose of parallel I/O}.\mknote{I want to say when we use parallel IO there is no simultaneous access to the same file anymore. When one process access it the others are not blocked otherwise why parallel IO should give us performance?}\gpnote{This is even more wierd. Parallel IO means concurrent access to a file but different parts of it. That is what MPI-IO does in the C level at least. Each process accesses a specific part of the file and all can access the whole file concurrently. There is no competing, because each process accesses a different part of it. In the end, when we observe the whole file all processes access it simultaneously.}\mknote{Ok I think we mean the same thing but I wrote it very badly previously I just improved it and wrote it in a more clear way now.}\gpnote{better feel free to remove these}
