\begin{abstract} 
\obnote{Streamline Abstract and make sure it reflects paper/especially it should echo Conclusions}
...
Our initial analysis shows that accessing a single file on the distributed file system leads to stragglers, and as a result, prevents any scaling beyond \gpnote{Is this still valid? It would better if you removed "a single node" and wrote "a level of distribution" or something similar}\mknote{Yes I believe it is valid. Because single-shared-file-IO from multiple nodes is only problematic. Single node does not have problem. Also, I do not understand what you mean by level of distribution.} a single node.
We introduce two important performance parameters $\overline{\tcomp}/\overline{\tIO}$ and $\overline{\tcomp}/\overline{\tcomm}$ which determines whether we observe any stragglers.
